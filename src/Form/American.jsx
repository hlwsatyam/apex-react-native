...
const American = ({}) => {
  ...
  const [formName, setFormName] = useState('American Bank');
  ...
  const saveHandler = async () => {
    try {
      ...
      await axios
        .post(`${baseAppUrl}/api/formSubmit/american`, {
          formName: formName,
          agentId: agentId,
          formData: agentInfo,
        })
      ...
      <ScrollView className="pt-3 bg-white p-5  min-h-screen rounded-tl-[70px] ">
        <View className="">
          {Object.entries(agentInfo).map(([key, value], index) => (
            <View key={index}>
              <Text className="text-black font-bold mt-10">
                {key} <Text className="text-red-400">*</Text>{' '}
              </Text>
              <TextInput
